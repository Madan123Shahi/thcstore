...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="product-card bg-white rounded-2xl shadow-card hover:shadow-card-hover transition-all duration-300 overflow-hidden group">
      ...
      <div className="product-img-wrapper relative bg-gray-50 aspect-square overflow-hidden">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          ...
          {product.isNewArrival && (
            <span className="badge bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full font-bold">New</span>
          )}
          {product.isBestSeller && (
            <span className="badge bg-earth-500 text-white text-xs px-2 py-0.5 rounded-full font-bold">Best Seller</span>
          )}
          {product.isAyushApproved && (
            <span className="badge bg-primary-600 text-white text-xs px-2 py-0.5 rounded-full font-bold">AYUSH ✓</span>
          )}
        </div>
        ...
        {product.stock === 0 && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
            <span className="bg-white text-gray-800 font-semibold text-sm px-4 py-2 rounded-full">Out of Stock</span>
          </div>
        )}
      </div>
      ...
      {/* Content */}
      <div className="p-4">
        <p className="text-xs text-primary-600 font-semibold uppercase tracking-wider mb-1">{product.brand}</p>
        <Link to={`/products/${product.slug || product._id}`}>
        ...
        {product.numReviews > 0 && (
          <div className="flex items-center gap-1 mb-2">
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <FiStar key={i} className={`text-xs ${i < Math.round(product.rating) ? 'text-amber-400 fill-current' : 'text-gray-200'}`} />
              ))}
            </div>
        ...
        <div className="flex items-center justify-between mt-3">
          <div>
            ...
            {product.mrp > product.price && (
              <span className="text-xs text-gray-400 line-through ml-1.5">{formatPrice(product.mrp)}</span>
            )}
          </div>
          <button
        ...
        {(product.volume || product.weight) && (
          <p className="text-xs text-gray-400 mt-1">{product.volume || product.weight}</p>
        )}
      </div>
    </div>
  );
