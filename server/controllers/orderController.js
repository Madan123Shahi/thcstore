...
// ─────────────────────────────────────────────
// ✅ Helper — emit order update to user + admin rooms
// ─────────────────────────────────────────────
...
export const refundOrder = asyncHandler(async (req, res) => {
  ...
  if (order.paymentMethod !== "stripe") throw new AppError("Refunds only apply to Stripe payments", 400);
  ...
  // ✅ Notify user in real time about refund
  emitOrderUpdate(order);
...
export const createOrder = asyncHandler(async (req, res) => {
  const { items, shippingAddress, paymentMethod, notes, paymentIntentId } = req.body;
  ...
  for (const item of items) {
    ...
    if (!product)
      throw new AppError(`Product not found or out of stock: ${item.product}`, 400);
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  // ✅ Emit real-time status update to customer and admin
  emitOrderUpdate(order);
