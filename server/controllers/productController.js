...
export const getProducts = asyncHandler(async (req, res) => {
  const {
    ...
    minPrice, maxPrice, brand, requiresPrescription,
  ...
  if (search) {
    const pipeline = [
      ...
      ...(brand ? [{ $match: { brand: new RegExp(brand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i") } }] : []),
      ...(requiresPrescription !== undefined ? [{ $match: { requiresPrescription: requiresPrescription === "true" } }] : []),
...
export const createProduct = asyncHandler(async (req, res) => {
  const {
    ...
    brand, tags, requiresPrescription,
    isFeatured, isBestSeller, isNewArrival,
  } = req.body;
  ...
  const uploadedImages = (req.files || []).map((file) => {
    ...
    return { url: file.path, thumbnail: urls.thumbnail, medium: urls.medium, large: urls.large, alt: name };
  });
...
export const updateProduct = asyncHandler(async (req, res) => {
  const {
    name, description, price, stock, category,
    brand, tags, requiresPrescription,
    isFeatured, isBestSeller, isNewArrival, existingImages,
  } = req.body;
  ...
  const product = await Product.findByIdAndUpdate(
    req.params.id,
    { name, description, price, stock, category, brand, tags, requiresPrescription, isFeatured, isBestSeller, isNewArrival, images },
