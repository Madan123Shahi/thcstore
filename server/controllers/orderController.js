...
// @access  Private
//
// FIX #1: Verify that the paymentIntent belongs to the
//         requesting user to prevent spoofing another user's
//         payment intent ID.
// ─────────────────────────────────────────────
export const verifyPayment = asyncHandler(async (req, res) => {
  ...
  // FIX #1: Ensure the payment intent belongs to the requesting user
...
// @access  Admin
//
// FIX #2: Use sendRefundEmail instead of sendShippingUpdateEmail
// ─────────────────────────────────────────────
export const refundOrder = asyncHandler(async (req, res) => {
  ...
  // FIX #2: Send refund-specific email, not shipping update email
...
// @access  Private
//
// FIX #3: Use MongoDB transaction to make stock decrement atomic
// FIX #4: Validate Stripe payment amount matches computed totalPrice
// FIX #5: Store discountAmount on the order document
// FIX #6: Only increment coupon usedCount if discount > 0
// ─────────────────────────────────────────────
export const createOrder = asyncHandler(async (req, res) => {
  ...
  // ── Use a MongoDB transaction for atomic stock decrement ────────────────
  // FIX #3: If any product fails mid-loop, all stock decrements are rolled back
  ...
  try {
    ...
    if (couponCode) {
      ...
      if (coupon) {
        ...
        if (valid) {
          ...
          // FIX #6: Only apply and count the coupon if the discount is > 0
          if (calculated > 0) {
    ...
    // FIX #4: Validate that Stripe payment amount matches the computed total
    if (paymentMethod === "stripe") {
    ...
    // FIX #5: Store discountAmount on the order so it's visible on invoices
    ...
    session.endSession();
    throw err; // Re-throw so asyncHandler/global error handler picks it up
  }
});
...
// @access  Admin
//
// FIX #7: Parse page and limit as integers to avoid
//         .skip() and .limit() receiving string values
// ─────────────────────────────────────────────
export const getAllOrders = asyncHandler(async (req, res) => {
  // FIX #7: Parse query params — Mongoose .skip()/.limit() require numbers
...
//       const { valid } = coupon.isValid(itemsPrice);
//       if (valid) {
//         discountAmount = coupon.calculateDiscount(itemsPrice, shippingPrice);
//         coupon.usedCount += 1; // ✅ increment usage
//         await coupon.save();
//       }
//     }
//   }
...
//   // ✅ Notify admin of new order in real time
//   io.to("admin").emit("order:new", {
//     orderId: order._id,
//     orderNumber: order.orderNumber,
