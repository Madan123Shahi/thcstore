...
import { FiMinus, FiPlus, FiHeart, FiShare2, FiShield, FiAlertCircle, FiChevronRight } from 'react-icons/fi';
...
import { PageLoader, StarRating, StarInput, ErrorMessage } from '../components/common';
...
export default function ProductDetailPage() {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    if (!reviewForm.comment.trim()) { toast.error('Please enter a comment'); return; }
    setSubmittingReview(true);
    const res = await dispatch(addReview({ id: product._id, data: reviewForm }));
  ...
  return (
    <div className="page-container py-8 animate-fade-in">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        {product.category && (
          <>
            <Link to={`/products?category=${product.category.slug}`} className="hover:text-primary-600 transition-colors">{product.category.name}</Link>
            <FiChevronRight className="text-xs" />
          </>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-14">
        ...
        <div className="space-y-3">
          <div className="aspect-square bg-gray-50 rounded-2xl overflow-hidden">
            <img src={getImageUrl(product.images?.[activeImg]?.url)} alt={product.name} className="w-full h-full object-contain p-4" />
          </div>
        ...
        <div>
          <div className="flex flex-wrap items-start gap-2 mb-2">
            {product.isAyushApproved && <span className="badge-green">AYUSH Approved</span>}
            {product.labTested && <span className="badge-hemp">Lab Tested</span>}
            {product.isBestSeller && <span className="badge-earth">Best Seller</span>}
            {product.isNewArrival && <span className="badge bg-blue-100 text-blue-700">New Arrival</span>}
          </div>

          <p className="text-primary-600 font-semibold text-sm uppercase tracking-wide mb-1">{product.brand}</p>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-gray-900 mb-3 leading-tight">{product.name}</h1>

          {product.numReviews > 0 && (
            <div className="mb-4">
              <StarRating rating={product.rating} reviews={product.numReviews} size="md" />
            </div>
          )}
          ...
          <div className="flex items-baseline gap-3 mb-4">
            ...
            {product.mrp > product.price && (
              <>
                ...
                <span className="badge bg-red-100 text-red-700 text-sm">{discount}% OFF</span>
              </>
            )}
          </div>
            ...
            <div className="flex gap-3 mb-5">
              {product.cbdContent && (
                <div className="bg-primary-50 border border-primary-100 rounded-xl px-4 py-2 text-center">
                  ...
                  <p className="font-bold text-primary-700">{product.cbdContent}</p>
                </div>
              )}
              {product.thcContent && (
                <div className="bg-earth-50 border border-earth-100 rounded-xl px-4 py-2 text-center">
                  ...
                  <p className="font-bold text-earth-700">{product.thcContent}</p>
                </div>
              )}
              {product.volume && (
          ...
          {product.requiresPrescription && (
            <div className="flex items-start gap-2 bg-orange-50 border border-orange-200 rounded-xl p-3 mb-5">
              ...
              <p className="text-sm text-orange-700">
                <strong>Prescription Required:</strong> This product requires a valid prescription from a licensed doctor. Please upload your prescription during checkout.
              </p>
            </div>
          )}
          ...
          {product.stock > 0 && (
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                <button onClick={() => setQty(q => Math.max(1, q - 1))} className="px-4 py-3 text-gray-600 hover:bg-gray-50 transition-colors">
                  <FiMinus />
                </button>
                <span className="px-5 py-3 font-semibold text-gray-900 min-w-[3rem] text-center">{qty}</span>
                <button onClick={() => setQty(q => Math.min(product.stock, q + 1))} className="px-4 py-3 text-gray-600 hover:bg-gray-50 transition-colors">
                  <FiPlus />
                </button>
              </div>
              <button onClick={() => handleAddToCart(product, qty)} className="btn-primary flex-1 py-3 text-base">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <div key={label} className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2 text-xs text-gray-600">
        ...
        <div className="flex gap-1">
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab} {tab === 'reviews' && product.numReviews > 0 && `(${product.numReviews})`}
            </button>
          ))}
        </div>
      </div>
        ...
        {activeTab === 'description' && (
          <div className="prose prose-green max-w-none">
            ...
            {product.features?.length > 0 && (
              <div className="mt-6">
                <h3 className="font-display font-bold text-gray-900 text-xl mb-3">Key Features</h3>
                <ul className="space-y-2">
                  {product.features.map((f, i) => (
                    <li key={i} className="flex items-start gap-2 text-gray-600 text-sm">
                      <FiShield className="text-primary-500 shrink-0 mt-0.5" /> {f}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {user && (
              <form onSubmit={handleReviewSubmit} className="bg-gray-50 rounded-2xl p-5 space-y-4">
                ...
                <textarea
                  ...
                  required
                />
                <button type="submit" disabled={submittingReview} className="btn-primary text-sm px-6 py-2.5">
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
                        <span className="text-primary-700 font-bold text-sm">{r.name?.[0]}</span>
                      </div>
