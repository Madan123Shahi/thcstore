  ...
  return (
    <AdminLayout title="Products">
      <div className="flex items-center justify-between gap-4 mb-5 flex-wrap">
        ...
        <Link to="/admin/products/new" className="btn-primary text-sm px-4 py-2.5 shrink-0">
          <FiPlus /> Add Product
        </Link>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wide hidden sm:table-cell">Brand</th>
                  ...
                  <th className="text-right px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wide hidden sm:table-cell">Stock</th>
                  ...
                  <tr key={p._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <img src={getImageUrl(p.images?.[0]?.url)} alt={p.name} className="w-10 h-10 rounded-lg object-cover bg-gray-100 shrink-0" />
                        <div className="min-w-0">
                          <p className="font-medium text-gray-800 line-clamp-1">{p.name}</p>
                          <div className="flex gap-1 mt-0.5">
                            {p.isFeatured && <span className="badge bg-blue-100 text-blue-700 text-[10px] px-1.5 py-0.5">Featured</span>}
                            {p.requiresPrescription && <span className="badge bg-orange-100 text-orange-700 text-[10px] px-1.5 py-0.5">Rx</span>}
                          </div>
                        </div>
                      </div>
                    </td>
          ...
          {pagination && pagination.pages > 1 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setPage(p => p + 1)} disabled={page === pagination.pages} className="btn-secondary px-3 py-1.5 text-xs disabled:opacity-40">Next →</button>
              </div>
            </div>
          )}
        </div>
