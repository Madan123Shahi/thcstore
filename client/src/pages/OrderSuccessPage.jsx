...
export default function OrderSuccessPage() {
  ...
  // ✅ After order loads, verify payment if it was a Stripe payment
  // This is a safety net — in case the webhook was delayed
  useEffect(() => {
    const verifyStripePayment = async () => {
      // Only verify Stripe orders that aren't already marked paid
      if (!order || order.paymentMethod !== "stripe" || order.isPaid) {
        setVerified(true); // COD or already paid — no need to verify
        return;
      }
      ...
      try {
        ...
        setVerified(true);
        // Re-fetch order to get updated isPaid status
        dispatch(fetchOrder(id));
        ...
        <div className="card p-6 text-left mb-6">
          ...
          <div className="mb-4">
            ...
            {order.paymentMethod === "cod" && (
              <span className="badge text-xs px-2 py-1 rounded-full font-medium bg-blue-100 text-blue-700 ml-2">
                💵 Cash on Delivery
              </span>
            )}
          </div>
