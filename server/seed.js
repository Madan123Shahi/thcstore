...
const seed = async () => {
  try {
    ...
    const Categories_With_Slug = CATEGORIES.map((c) => ({
      ...c,
      slug: slugify(c.name, { lower: true }),
    }));

    // Seed categories
    const cats = await Category.insertMany(Categories_With_Slug);
    ...
    const PRODUCTS = [
      ...
      },
      {
        ...
        images: [
          {
            url: "https://placehold.co/400x400/bbf7d0/15803d?text=CBD+Gummies",
            alt: "CBD Gummies",
          },
        ],
        features: [
      ...
      },
      {
        ...
        images: [
          {
            url: "https://placehold.co/400x400/dbeafe/1e40af?text=Pet+CBD",
            alt: "Pet CBD Oil",
          },
        ],
        features: [
      ...
      },
      {
        ...
        images: [
          {
            url: "https://placehold.co/400x400/fce7f3/9d174d?text=CBD+Cream",
            alt: "CBD Cream",
          },
        ],
        features: [
    ...
    const PRODUCTS_WITH_SLUG = PRODUCTS.map((p) => ({
      ...p,
      slug: slugify(p.name, { lower: true }),
    }));

    const products = await Product.insertMany(PRODUCTS_WITH_SLUG);
