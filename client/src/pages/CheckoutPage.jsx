...
// ✅ Load Stripe outside component — prevents re-instantiation on re-render
...
const PAYMENT_METHODS = [
  ...
  },
  {
    ...
    label: "Card Payment",
    icon: "💳",
    desc: "Visa, Mastercard, Amex securely via Stripe",
  },
];
...
function CheckoutForm({
  ...
  const [step, setStep] = useState(1); // starts at payment step
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // 2. Confirm card payment on Stripe's side
      ...
      // 3. Payment succeeded — create order in our DB with paymentIntentId
      const res = await dispatch(
        createOrder({
          items: items.map((i) => ({
            product: i.product._id,
          ...
          paymentIntentId: paymentIntent.id, // ✅ backend verifies this
        }),
      );
  ...
  return (
    <div className="card p-6 space-y-5">
      ...
      {/* Stripe card input — only shown when stripe is selected */}
      {paymentMethod === "stripe" && (
        <div>
    ...
    <div className="page-container py-8 animate-fade-in">
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            ...
            <span
              className={`text-sm font-medium hidden sm:block ${i === step ? "text-primary-700" : "text-gray-400"}`}
            >
              {s}
            </span>
            {i < STEPS.length - 1 && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
              ...
              {user?.addresses?.length > 0 && (
                <div className="mb-4">
                  ...
                  <div className="my-4 flex items-center gap-3 text-xs text-gray-400">
                    <div className="flex-1 h-px bg-gray-200" /> or add new{" "}
          ...
          {/* Step 2: Review + Place Order — wrapped in Stripe Elements */}
          {step === 2 && (
